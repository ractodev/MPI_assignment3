...
int main(int argc, char *argv[]){
    ...
    MPI_Send(&f[1]        , 1, MPI_DOUBLE, (rank+size-1)%size, 0, MPI_COMM_WORLD);
    MPI_Recv(&f[nxn_loc-1], 1, MPI_DOUBLE, (rank+size+1)%size, 0, MPI_COMM_WORLD, &status);
    MPI_Send(&f[nxn_loc-2], 1, MPI_DOUBLE, (rank+size+1)%size, 1, MPI_COMM_WORLD);
    MPI_Recv(&f[0]        , 1, MPI_DOUBLE, (rank+size-1)%size, 1, MPI_COMM_WORLD, &status);  
